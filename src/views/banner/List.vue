<template>
  <el-row>
  <el-col :span="24">
  	<div style="font-size: 16px;margin-top: 10px;">Banner列表 记录（）</div> 
  </el-col>
...
// 审核不通过
      async CheckFShoolinfo () {
        ...
        if (count !== 1) {
          ...
          let data = await http.post('/banner/ediCheckFlag', shoolinfoIDs)
          this.$message({
            message: '审核不通过' + data.data.obj,
            type: 'success'
          })
          this.onSubmit()
        } else {
          this.$message.error('请选择已审核的')
        }
      },
